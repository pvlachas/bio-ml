"""
Training script for DDPM with learned mean and variance.

This experiment trains a diffusion model that learns both the mean and
variance of the denoising distribution.
"""
...
for epoch in epoch_iterator:
    ...
    if epoch % REPORT_EVERY == 0:
        with torch.no_grad():
            ...
            _, pred_logvar = denoising_model_mean_variance(model, sample_xt, sample_t)
    ...
    if epoch % EVAL_EVERY == 0:
        ...
        samples_traj, samples_final = config['sampling_fn'](
            model, shape=(SAMPLES_EVAL, 2), n_steps=N_STEPS, device=device
        )
